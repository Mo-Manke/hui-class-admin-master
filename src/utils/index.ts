...
/**
 * 把日期范围构造成 beginTime AND endTime,并且去除空白的值
 * @param searchParams
 ...
 * @returns
 */
export function koiDatePicker(searchParams: any, dateRange: any, propName1?: string, propName2?: string) {
    searchParams = typeof searchParams === "object" && searchParams !== null && !Array.isArray(searchParams) ? searchParams : {};
    Object.keys(searchParams).forEach(key => {
        if ((!searchParams[key] || searchParams[key] == "") && typeof searchParams[key] != "boolean" && searchParams[key] !== 0) {
...
export function getShowStaticAndDynamicMenuList(menuList: any) {
    ...
    const filterTree = (list: any[]) =>
        list
            .filter((item: any) => item.isHide == "1" || item.meta?.isHide == "1")
            .map((item: any) => {
                const next = { ...item };
                if (item.children) {
                    next.children = filterTree(item.children);
                }
                return next;
            });
    return filterTree(newMenuList);
}
